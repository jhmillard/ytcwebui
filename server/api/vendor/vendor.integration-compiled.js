...
describe('Vendor API:', function () {
  ...
  describe('GET /api/vendors', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/vendors').expect(200).expect('Content-Type', /json/).end((err, res) => {
        if (err) {
          return done(err);
        }
        vendors = res.body;
  ...
  describe('POST /api/vendors', function () {
    beforeEach(function (done) {
      request(app).post('/api/vendors').send({
        name: 'New Vendor',
  ...
  describe('GET /api/vendors/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/vendors/' + newVendor._id).expect(200).expect('Content-Type', /json/).end((err, res) => {
        if (err) {
          return done(err);
        }
        vendor = res.body;
  ...
  describe('PUT /api/vendors/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).put('/api/vendors/' + newVendor._id).send({
        name: 'Updated Vendor',
  ...
  describe('DELETE /api/vendors/:id', function () {
    ...
    it('should respond with 204 on successful removal', function (done) {
      request(app).delete('/api/vendors/' + newVendor._id).expect(204).end((err, res) => {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when vendor does not exist', function (done) {
      request(app).delete('/api/vendors/' + newVendor._id).expect(404).end((err, res) => {
        if (err) {
          return done(err);
        }
        done();
